...
#                                     от базового для тестов (для assertEqual и тп)
    ...
    def test_area_positive(self): #создала метод-тест от текущего прототипа класса
        """Проверка площади круга с положительным радиусом"""
        self.assertAlmostEqual(area(1), math.pi) #обращаемся к методу текущего прототипа класса, вызываем функцию, которая должна возвращать дробное значение, поэтому используем специальное сравнение
